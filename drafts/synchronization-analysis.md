...
## 8. 関連コードと公開前確認 (Review Notes)

この草稿は、次のファイルをもとに整理した。
...
- 授業内の班実験であることの表現が適切か。
- コードを GitHub で公開する場合、文末に GitHub project card を追加するか。
- `src/content/works/` に入れる際、`featured: true` のままホームページに出してよいか。
- 日本語本文の文体をさらに簡潔にするか。
