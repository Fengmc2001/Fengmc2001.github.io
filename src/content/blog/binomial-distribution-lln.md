...
$$ Y_1 = \sum_{i=1}^{12} X_i = 6 $$
$$ \text{VAR}[Y_1] = \frac{1}{12} \times 12 = 1 $$

さらに $Y_1$ から6を引くと、期待値は以下のように計算される。
$$ Y = 6 - 6 = 0 $$

分散は確率変数から定数を加減しても変わらないため、以下の通りである。
$$ \text{VAR}[Y] = \text{VAR}[Y-6] = \text{VAR}[Y] = 1 $$

以上の導出により、$Y$ は $Y \sim N(0, 1)$ の正規分布（標準正規分布）に従うことが証明される。
