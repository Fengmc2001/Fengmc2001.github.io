...
計算の結果、1 群あたり約 43 名、計 86 名が必要と算出された。この数字は「$n = 5$ では power が著しく不足しており、$p = 0.788$ という結果はサンプルサイズ不足による可能性が高い」ことを裏付けている。power analysis の実践的価値は、「有意差なし」という結果に直面したとき、それが「効果がないから」なのか「サンプルが足りないから」なのかを区別する根拠を与える点にある。
