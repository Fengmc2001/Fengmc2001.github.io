...
年齢と投与前 SBP に $r = 0.92$、年齢と投与後 SBP に $r = 0.93$ と非常に強い正の相関が認められた。投与前 SBP と投与後 SBP の相関も $r = 0.91$ と高く、ベースライン値の個人差が投与後の値にも大きく影響する構造が見える。このことは「投与後の血圧」単独での群間比較より、「投与前後の変化量」を評価する方がベースラインの個人差の影響を軽減できることを示唆している。
...
- $t = -6.05$, $\text{df} \approx 5.6$, $p = 0.0012$
- 95% 信頼区間：$[-20.03, -8.37]$
...
- $t = -2.75$, $\text{df} \approx 4.3$, $p = 0.0478$
- 95% 信頼区間：$[-30.17, -0.23]$
...
予備実験で想定した群間差 $\Delta = 8$ とよく一致する（実測差は約 9.6 mmHg）結果が得られた。統計学的に有意な降圧効果が確認された。効果量（約 9 mmHg）は想定した $\Delta = 8$ と整合する臨床的に意味のある水準だが、本データは模擬データである点には留意が必要である。
